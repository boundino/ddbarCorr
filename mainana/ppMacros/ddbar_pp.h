...
namespace ddbar
{
  ...
  {
  public:
    kinematics(float pt1min, float pt1max, float pt2min, float pt2max, float yd) : 
      fpt1min(pt1min), fpt1max(pt1max), fpt2min(pt2min), fpt2max(pt2max), fyd(yd) { ; }
  ...
  private:
    float fpt1min, fpt1max, fpt2min, fpt2max, fyd;
